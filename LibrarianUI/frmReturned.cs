    ...
    public partial class frmReturned : Form
    {
        ...
        private void btnNew_Click(object sender, EventArgs e)
        {
            sql = "SELECT br.`BorrowerId`, `BookTitle`, `DateBorrowed`, `Purpose`, `DueDate`,BorrowId,br.AccessionNo " +
                  ...
                  " WHERE br.AccessionNo=b.AccessionNo AND br.`BorrowerId`=bw.`BorrowerId` AND br.Status='Borrowed' AND Due=0 AND br.`BorrowerId` LIKE '%" +
                  txtSearchPborrower.Text + "%' ORDER BY BorrowId Desc";
        ...
        {
            sql =
                "SELECT bw.`BorrowerId`, `Firstname`, `Lastname`,DateBorrowed,b.`AccessionNo`,`BookTitle`, `BookDesc`,`DateReturned` " +
                ...
                " AND  (bw.`BorrowerId` Like '%" + txtrbooksSearch.Text + "%' OR b.`AccessionNo` Like '%" +
                txtrbooksSearch.Text +
        ...
        private void dtg_RlistReturn_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            sql = "SELECT *  " +
                  " FROM `tblborrow` br,`tblbooks` b,`tblborrower` bw  " +
                  " WHERE br.AccessionNo=b.AccessionNo AND br.`BorrowerId`=bw.`BorrowerId` AND BorrowId=" +
                  dtg_RlistReturn.CurrentRow.Cells[5].Value;
            config.singleResult(sql);
            if (config.dt.Rows.Count > 0)
            {
                txtRname.Text = config.dt.Rows[0].Field<string>("Firstname") + " " +
        ...
        private void btn_Rsave_Click(object sender, EventArgs e)
        {
            if (txtRname.Text == "")
            {
                MessageBox.Show("There is no borrower in the fields.", "Invalid", MessageBoxButtons.OK,
            ...
            sql = "UPDATE `tblborrow` SET  `Status` =  'Returned',`Remarks`='On Time' WHERE `BorrowId` = '" +
                  dtg_RlistReturn.CurrentRow.Cells[5].Value + "'";
            config.Execute_Query(sql);


            sql = "UPDATE  `tblbooks` SET  `Status` =  'Available' WHERE  `AccessionNo` = '" +
                  dtg_RlistReturn.CurrentRow.Cells[6].Value + "'";
            config.Execute_Query(sql);
