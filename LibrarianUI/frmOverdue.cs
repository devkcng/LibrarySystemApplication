...
namespace LibrarianUI
{
    public partial class frmOverdue : Form
    {
        ...
        private void btnPenNew_Click(object sender, EventArgs e)
        {
            ...
            sql =
                "SELECT br.`BorrowerId`, `BookTitle`,`DateBorrowed`, `Purpose`, `DueDate` , BorrowId,br.AccessionNo " +
        ...
        private void txtSearchPborrower_TextChanged(object sender, EventArgs e)
        {
            sql =
                "SELECT br.`BorrowerId`, `BookTitle`,`DateBorrowed`, `Purpose`, `DueDate` , BorrowId,br.AccessionNo " +
                " FROM `tblborrow` br,`tblbooks` b,`tblborrower` bw  " +
                " WHERE br.AccessionNo=b.AccessionNo AND br.`BorrowerId`=bw.`BorrowerId` AND br.Status='Borrowed' AND Due=1 AND   br.`BorrowerId` Like '%" +
                txtSearchPborrower.Text + "%'";
            config.Load_ResultList(sql, dtgPenalties);
            dtgPenalties.Columns[5].Visible = false;
            dtgPenalties.Columns[6].Visible = false;
        }
        ...
        private void dtgPenalties_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            sql = "SELECT  TIMEDIFF( NOW( ) ,  `DueDate` ),`AccessionNo` "
                  + "FROM  `tblborrow` WHERE `BorrowId` = '" + dtgPenalties.CurrentRow.Cells[5].Value + "'";
            config.singleResult(sql);
            if (config.dt.Rows.Count > 0)
            {
                var ts = config.dt.Rows[0].Field<TimeSpan>(0);
        ...
        private void btnPSave_Click(object sender, EventArgs e)
        {
            ...
            sql = "INSERT INTO `tblpayment` (`BorrowId`, `Payment`, `Change`, `DatePayed`, `BorrowerId`, `Remarks`) "
                  + " VALUES (" + dtgPenalties.CurrentRow.Cells[6].Value + ",'" + txtTotPay.Text + "','" +
                  txtChange.Text +
            ...
            sql = "UPDATE  `tblbooks` SET  `Status` =  'Available' WHERE  `AccessionNo` = '" +
                  dtgPenalties.CurrentRow.Cells[6].Value + "'";
            config.Execute_Query(sql);

            sql = "UPDATE `tblborrow` SET  `Status` =  'Returned', Due = 0 WHERE `BorrowId` = " +
                  dtgPenalties.CurrentRow.Cells[5].Value;
            config.Execute_Query(sql);
