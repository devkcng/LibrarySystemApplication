...
namespace LibrarianUI.Includes
{
    internal class SQLConfig
    {
        private readonly MySqlConnection con =
            new MySqlConnection("server=localhost;user id=root;password=;database=dblibrary;sslMode=none");
        ...
        public void combo(string sql, ComboBox cbo)
        {
            try
            {
                ...
                cbo.Items.Clear();
                cbo.Text = "Select";
                foreach (DataRow r in dt.Rows) cbo.Items.Add(r.Field<string>(0));
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message);
        ...
        public void autocomplete(string sql, TextBox txt)
        {
            try
            {
                ...
                foreach (DataRow r in dt.Rows) txt.AutoCompleteCustomSource.Add(r.Field<string>(0));
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message);
        ...
        public void autonumber(string AUTOKEY, TextBox txt)
        {
            try
            {
                ...
                cmd.Connection = con;
                cmd.CommandText = "SELECT CONCAT(`Autonumber`,`EndValue`) FROM `tblautonumber` WHERE Remarks='" +
                                  AUTOKEY + "'";
                da.SelectCommand = cmd;
                da.Fill(dt);
