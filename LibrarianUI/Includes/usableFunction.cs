...
namespace LibrarianUI.Includes
{
    internal class usableFunction
    {
        ...
        private static Regex Valid_Name = StringOnly();
        private static Regex Valid_Contact = NumbersOnly();
        private static Regex Valid_Password = ValidPassword();
        private static Regex Valid_Email = Email_Address();
            ...
            try
            {
                ...
                foreach (Control txt in container.Controls)
                {
                    ...
                    if (txt is TextBox)
                        ...
                        txt.Text = "";
                    if (txt is RichTextBox) txt.Text = "";
                }
            }
            catch (Exception ex)
            {
        ...
        {
            var StringAndNumber_Pattern = "^[a-zA-Z]";
        ...
        {
            var StringAndNumber_Pattern = "^[0-9]*$";
        ...
        {
            var Password_Pattern = "(?!^[0-9]*$)(?!^[a-zA-Z]*$)^([a-zA-Z0-9]{8,15})$";
        ...
        {
            MessageBox.Show("There are empty fields left that must be filled up!", "Invalid", MessageBoxButtons.OK,
