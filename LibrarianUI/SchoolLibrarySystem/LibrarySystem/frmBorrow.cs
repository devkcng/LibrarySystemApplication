...
namespace LibrarySystem
{
    public partial class frmBorrow : Form
    {
        ...
        private void txtAccesionNumBorrow_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                sql = "Select * From tblbooks b, tblcategory c WHERE b.CategoryId=c.CategoryId AND AccessionNo='" +
        ...
        private void btnNew_Click(object sender, EventArgs e)
        {
            ...
            sql =
                " SELECT br.`AccessionNo`, `BookTitle`, `BookDesc` as 'Description',Concat(`Firstname`,' ', `Lastname`) as 'Borrower',`NoCopies`, `DateBorrowed`, `Purpose`, `DueDate` FROM `tblborrow` br,`tblbooks` b,`tblborrower` bw  WHERE br.AccessionNo=b.AccessionNo AND br.`BorrowerId`=bw.`BorrowerId` ORDER BY BorrowId Desc";
        ...
        private void txtBorrowerId_TextChanged(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if (config.dt.Rows.Count > 0)
                {
                    txtName.Text = config.dt.Rows[0].Field<string>("Firstname") + " " +
        ...
        private void btn_Bsave_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("All fields are required.", "Invalid", MessageBoxButtons.OK,
            ...
            {
                MessageBox.Show("Pls. choose your porpuse.", "Invalid", MessageBoxButtons.OK,
                    MessageBoxIcon.Exclamation);
                return;
            }
            ...
            sql = "INSERT INTO  `tblborrow`  " +
                  ...
                  " VALUES ('" + txtAccesionNumBorrow.Text + "',1,NOW(),'" + cboPurpose.Text + "', 'Borrowed','" +
                  txtdue.Text + "','" + txtBorrowerId.Text + "')";
            config.Execute_CUD(sql, "error to execute query.", "Book has been borrowed in the library");
            config.Execute_Query("UPDATE tblbooks set Status = 'Not Available' Where AccessionNo='" +
                                 txtAccesionNumBorrow.Text + "'");
        ...
        {
            sql =
                " SELECT br.`AccessionNo`, `BookTitle`, `BookDesc` as 'Description',Concat(`Firstname`,' ', `Lastname`) as 'Borrower', `DateBorrowed`, `Purpose`, `DueDate` " +
                " FROM `tblborrow` br,`tblbooks` b,`tblborrower` bw  " +
                " WHERE br.AccessionNo=b.AccessionNo AND br.`BorrowerId`=bw.`BorrowerId` AND (BookTitle Like '%" +
                txt_Search.Text + "' or br.AccessionNo Like '%" + txt_Search.Text +
                "%' OR Concat(`Firstname`,' ', `Lastname`)  Like '%" + txt_Search.Text + "') ORDER BY BorrowId Desc";
            config.Load_ResultList(sql, dtg_BlistOfBooks);
        }
    }
}